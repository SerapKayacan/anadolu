    ...
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            <div id="kt_app_content_container" class="app-container container-xxl">
                <form class="form d-flex flex-column flex-lg-row" action="{{ route('service.update', $service->id) }}" method="POST" id="form" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                                        ...
                                        <div class="card-body pt-5">
                                                        ...
                                                        <div class="input-group">
                                                            <span class="input-group-btn">
                                                                <a class="uploadImage btn btn-primary text-white btn-sm">
                                                                   ...
                                                                   <input type="text" name="uploaded_banner_image" class="d-none" value="{{ $service->getFirstMediaUrl('banner', 'large') }}">
                                                                ...
                                                                <div class="row col-md-12 thumb-output p-2">
                                                                    <img class="thumb img-thumbnail" src="{{ $service->getFirstMediaUrl('banner', 'large') }}">
                                                                </div>
                                                            </span>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="mb-10 row">
                                                <div class="col-sm-4">
                                                    ...
                                                    <select name="category_id" class="form-control col-sm-4" required>
                                                        @foreach($categories as $category)
                                                            <option @if($category->id == $service->category_id) selected @endif value="{{$category->id}}">{{ $category->title }}</option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col-sm-4">
                                                    ...
                                                    <select name="can_be_appointment" id="can_be_appointment" class="form-control" required>
                                                        <option @if ($service->can_be_appointment == 1) selected @endif value="1">Evet</option>
                                                        <option @if ($service->can_be_appointment == 0) selected @endif value="0">Hayır</option>
                                                    </select>
                                                    <div class="text-muted fs-7">Bu alan zorunludur.</div>
                                                </div>
                                                    ...
                                                    <div class="d-flex">
                                                        <select class="form-control me-2" id="appointment_start_time" name="appointment_start_time">
                                                            @foreach (range(0, 23) as $hour)
                                                                <option value="{{ sprintf('%02d:00', $hour) }}" @if(\Carbon\Carbon::parse($service->appointment_start_time)->format('H:i') == sprintf('%02d:00', $hour)) selected @endif>{{ sprintf('%02d:00', $hour) }}</option>
                                                                <option value="{{ sprintf('%02d:30', $hour) }}" @if(\Carbon\Carbon::parse($service->appointment_start_time)->format('H:i') == sprintf('%02d:30', $hour)) selected @endif>{{ sprintf('%02d:30', $hour) }}</option>
                                                            @endforeach
                                                        </select>
                                                        <select class="form-control" id="appointment_end_time" name="appointment_end_time">
                                                            @foreach (range(0, 23) as $hour)
                                                                <option value="{{ sprintf('%02d:00', $hour) }}" @if(\Carbon\Carbon::parse($service->appointment_end_time)->format('H:i')  == sprintf('%02d:00', $hour)) selected @endif>{{ sprintf('%02d:00', $hour) }}</option>
                                                                <option value="{{ sprintf('%02d:30', $hour) }}" @if(\Carbon\Carbon::parse($service->appointment_end_time)->format('H:i')  == sprintf('%02d:30', $hour)) selected @endif>{{ sprintf('%02d:30', $hour) }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    <div class="text-muted fs-7">Bu alan zorunludur.</div>
                                            ...
                                            <div class="mb-10 row">
                                                <div class="col-sm-12">
                                                    ...
                                                    <textarea class="form-control" name="category_page_detail" rows="2">{{ $service->category_page_detail }}</textarea>
                                            ...
                                            <div class="mb-10 row">
                                                <div class="col-sm-12">
                                                    ...
                                                    <textarea class="form-control" name="sort_detail" rows="2">{{ $service->sort_detail }}</textarea>
                                                    <div class="text-muted fs-7">Bu alan zorunlu değildir.</div>
                                                </div>
                                            </div>
                                            <div class="mb-10 row">
                                                <div class="col-sm-12">
                                                    ...
                                                    <textarea class="form-control" name="detail" rows="2">{{ $service->detail }}</textarea>
                                                    <div class="text-muted fs-7">Bu alan zorunlu değildir.</div>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-sm-3">
                                                    ...
                                                    <input class="form-control" name="sort_order" type="number" value="{{ $service->sort_order }}" min="0" max="99"/>
                                                    <div class="text-muted fs-7">Bu alan zorunludur.</div>
                                                </div>
                                                <div class="col-sm-3">
                                                    <label class="required form-label">Durum</label>
                                                    <select name="is_active" class="form-control col-sm-3" required>
                                                        <option @if ($service->is_active == 1) selected @endif value="1">Aktif</option>
                                                        <option @if ($service->is_active == 0) selected @endif value="0">Pasif</option>
                                                    </select>
                                                    <div class="text-muted fs-7">Bu alan zorunludur.</div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="card-body pt-5">
                                            <div class="mb-10 row">
                                                <div class="col-sm-12">
                                                    ...
                                                    <input class="form-control" name="meta_description" value="{{ $service->meta_description }}">
                                                    <div class="text-muted fs-7">Bu alan zorunlu değildir.</div>
                                                </div>
                                            </div>
                                            <div class="mb-10 row">
