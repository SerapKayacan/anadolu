                    ...
                    <div class="card-body pt-0">
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="dataTable">
                            ...
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-250px">Hizmet Kategori Listesi</th>
                            ...
                            @foreach ($contents as $content)
                                <tr id="order-{{ $content->id }}">
                                    <td>
                                        {{ $loop->iteration }}
                                    </td>
                                    <td>
                                        <a href="{{ route('service-category.edit', $content->id) }}" class="text-gray-800 text-hover-primary fs-5 fw-bold mb-1">{{ $content->title }}</a>
                                    </td>
                                    <td>
                                        {{ $content->getCategory?->name }}
                                    </td>
                                    <td>
                                        <a href="{{ $content->getFirstMediaUrl('banner', 'small') }}" target="_blank"><img src="{{ $content->getFirstMediaUrl('banner', 'small') }}" class="img-fluid"></a>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('slug',$content->slug) }}" data-toggle="tooltip" title="Görüntüle" class="btn btn-icon btn-light btn-hover-primary btn-sm" target="_blank"><i class="far fa-eye text-dark icon-lg"></i></a>
                                        <a href="{{ route('service-category.edit', $content->id) }}" data-toggle="tooltip" title="Düzenle" class="btn btn-icon btn-light btn-hover-primary btn-sm"><i class="far fa-edit text-warning icon-lg"></i></a>
                                        <a href="#" type="button" data-toggle="tooltip" title="Sil" class="btn btn-icon btn-light btn-hover-primary btn-sm"
                                           data-url="{{ route('service-category.destroy',$content->id) }}"
                                           data-method="delete"
                                           data-id="{{ $content->id }}"
...
@section('script')
    ...
    <script src="{{ asset('') }}assets/js/plm/datatable.js"></script>
    <script src="{{ asset('') }}assets/js/plm/delete.js"></script>
@endsection
